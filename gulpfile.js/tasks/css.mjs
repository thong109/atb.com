...
const cssResultFileStream = fs.createWriteStream(cssResultFilePath, {
  flags: "w",
});
...
const customLint = (root, result) => {
  root.walk((node) => {
    if (node.type === "atrule") {
      if (
        (node.name === "media" || node.name === "include") &&
        node.parent.type === "root"
      ) {
        if (node.params !== "print") {
          result.warn(
            `Do not use @media or @include in root-level. Found: @${node.name}`,
            {
              node: node,
            },
          );
      ...
      if (node.value.indexOf("'") !== -1) {
        result.warn(
          `Use double quotes instead of single quotes in SCSS files. Property: ${node.prop}`,
          {
            node: node,
          },
        );
      ...
      selectors.forEach((selector) => {
        if (selector.includes(":not")) {
          result.warn(
            `Avoid using :not() selector in SCSS. Selector: ${selector}`,
            {
              node: node,
            },
          );
...
const lintStyles = (fix = false) => {
  ...
  let filesToCheck = [config.css.path.inputFiles]
  ...
  return src([config.css.path.inputFiles])
              ...
              if (node.type === "decl") {
                ...
                if (
                  ...
                  (/#|rgba?|hsla?/i.test(node.value) ||
                    colorVariableNames.some((variable) =>
                      node.value.includes(variable),
                    ))
                ) {
                  if (!node.prop.startsWith("$")) {
                    colorValues.push({
                ...
                if (
                  node.prop === "height" ||
                  node.prop === "max-height" ||
                  node.prop === "min-height"
                ) {
                  heightValues.push({
                    fileName,
                    filePath,
    ...
    .on("end", function () {
      definedVariables.forEach((variable) => {
        variable.count = sortedVariables.find(
    ...
    .pipe(
      cheerio({
        run: function ($) {
          ...
          cssClassList = cssClassList.map((className) =>
            className.replace(/^\./, ""),
          );
        },
        parserOptions: {
...
export { lintStyles, lintStylesFix, buildStyles, css, analyze, compareClasses };
